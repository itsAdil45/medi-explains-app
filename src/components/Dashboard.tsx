...
import { Plus, Search, ChevronRight, AlertTriangle, Clock, CheckCircle2 } from "lucide-react-native";
import { api } from "@/api/client";
import { useAuth } from "@/api/auth";
import { StatusBadge } from "@/components/ui/badge";

const NEEDS_ATTENTION = ["safety_review_required", "drafted", "processing_failed", "rejected"];
...
const LANG_LABEL: Record<string, string> = { en: "EN", ur: "UR", ar: "AR", pa_shah: "PA", ps: "PS", sd: "SD" };
...
function groupOf(item: any, isTreatingDoctor: boolean) {
  ...
  if (isTreatingDoctor && item.report_uploaded_at && !item.report_reviewed_at) return "attention";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (showsDoctorColumn) {
      api
        .listDoctors()
        .then((list) => setPeopleById(Object.fromEntries(list.map((d: any) => [d.id, d]))))
        .catch(() => {});
    } else if (showsPatientColumn) {
      api
        .listPatients()
        .then((list) => setPeopleById(Object.fromEntries(list.map((p: any) => [p.id, p]))))
        .catch(() => {});
    }
  ...
  const counts = useMemo(() => {
    ...
    for (const item of items) c[groupOf(item, isTreatingDoctor) as keyof typeof c]++;
    return c;
  }, [items, isTreatingDoctor]);

  const filtered = useMemo(() => {
    return items
      .filter((item) => tab === "all" || groupOf(item, isTreatingDoctor) === tab)
      .filter((item) => {
  ...
  return (
    <ScrollView className="flex-1 bg-white px-5 py-6">
      <View className="mb-5 flex-row items-start justify-between">
        <View>
          <Text className="mb-1 text-xl font-bold tracking-tight text-slate-900">{title}</Text>
          <Text className="text-[13px] text-slate-500">
            {items.length} total
            {counts.attention > 0 && ` \u00b7 ${counts.attention} need${counts.attention === 1 ? "s" : ""} your attention`}
          </Text>
        </View>
        {(user.role === "doctor" || user.role === "admin") && (
          ...
          >
            ...
            <Text className="text-sm font-semibold text-white">New consultation</Text>
          </Pressable>
        )}
      </View>
      ...
      {items.length > 0 && (
        <View className="mb-5 flex-row gap-3">
          <View className="flex-1 flex-row items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4">
            ...
            <View>
              <Text className="text-lg font-bold leading-tight text-slate-900">{counts.attention}</Text>
          ...
          <View className="flex-1 flex-row items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4">
            ...
            <View>
              <Text className="text-lg font-bold leading-tight text-slate-900">{counts.progress}</Text>
          ...
          <View className="flex-1 flex-row items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4">
            ...
            <View>
              <Text className="text-lg font-bold leading-tight text-slate-900">{counts.released}</Text>
        ...
        <View className="mb-3.5 gap-3">
          <View className="flex-row gap-1 rounded-lg bg-slate-100 p-[3px]">
            {TABS.map((t) => (
              <Pressable
                ...
                className={`flex-1 items-center rounded-md px-2 py-1.5 ${tab === t.key ? "bg-white shadow-sm" : ""}`}
              >
                <Text className={`text-xs font-semibold ${tab === t.key ? "text-slate-900" : "text-slate-500"}`}>
                  {t.label}
                </Text>
              </Pressable>
            ))}
          ...
          <View className="flex-row items-center rounded-lg border border-slate-200 px-2.5">
            ...
            <TextInput
              ...
              className="ml-2 flex-1 py-2 text-[13px] text-slate-900"
            />
          </View>
        </View>
      )}
      ...
      {err && <Text className="py-8 text-sm font-medium text-red-600">{err}</Text>}
      ...
      {!loading && !err && items.length > 0 && filtered.length === 0 && (
        <View className="items-center rounded-xl border border-dashed border-slate-300 py-14">
          <Text className="text-sm text-slate-500">No consultations match this filter.</Text>
        </View>
      )}
          ...
          <Link key={c.id} href={`/consultation/${c.id}`} asChild>
            <Pressable
              className={`flex-row items-center gap-3 rounded-[11px] border bg-white px-4 py-3.5 ${
                c.status === "processing_failed" ? "border-red-200" : "border-slate-200"
              }`}
            >
              <View className="size-9 items-center justify-center rounded-full bg-slate-100">
                <Text className="text-xs font-bold text-slate-600">
                  {LANG_LABEL[c.patient_language] || c.patient_language?.slice(0, 2).toUpperCase()}
                </Text>
              </View>
              <View className="min-w-0 flex-1">
                <View className="flex-row items-center gap-2">
                  <Text className="text-sm font-semibold text-slate-900">Consultation #{c.id}</Text>
                  <StatusBadge status={c.status} />
                </View>
                {showsDoctorColumn && peopleById[c.doctor_id] && (
                  <Text className="mt-0.5 text-xs font-medium text-slate-700" numberOfLines={1}>
                ...
                {showsPatientColumn && peopleById[c.patient_id] && (
                  <Text className="mt-0.5 text-xs font-medium text-slate-700" numberOfLines={1}>
                    ...
                    {peopleById[c.patient_id].email && ` \u00b7 ${peopleById[c.patient_id].email}`}
                    {peopleById[c.patient_id].phone && ` \u00b7 ${peopleById[c.patient_id].phone}`}
                  </Text>
                )}
                <Text className="mt-0.5 text-xs text-slate-500">
                  {c.patient_language?.toUpperCase()} {"\u00b7"} {new Date(c.created_at).toLocaleString()}
                </Text>
              </View>
